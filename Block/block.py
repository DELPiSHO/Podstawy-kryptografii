...
def imagencryptecb():
    ...
    asyncio.subprocess.getoutput(cecba)
    cecbb1 = "of=letter-ecb.bmp bs=1 "
    cecbb2 = "count=54 conv=notrunc"
    cecbb = cb0 + cecbb1 + cecbb2
    asyncio.subprocess.getoutput(cecbb)

def imagencryptcbc():
    ccbca1 = "enc -aes-256-cbc -in letter-plain.bmp "
    ccbca2 = "-out letter-cbc.bmp -pass pass:qwertyuiop"
    ccbca = ca0 + ccbca1 +  ccbca2
    asyncio.subprocess.getoutput(ccbca)
    ccbcb1 = "of=letter-cbc.bmp bs=1 "
    ccbcb2 = "count=54 conv=notrunc"
    ccbcb = cb0 + ccbcb1 + ccbcb2
    asyncio.subprocess.getoutput(ccbcb)
    getstatusoutput("openssl enc -aes-256-cbc -in letter-plain.bmp -out letter-cbc.bmp -pass pass:mypass")
    asyncio.subprocess.getstatusoutput("dd if=letter-plain.bmp of=letter-cbc.bmp bs=1 count=54 conv=notrunc")
